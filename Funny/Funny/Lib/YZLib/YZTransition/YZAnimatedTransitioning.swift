...
class YZAnimatedTransitioning: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    fileprivate func custom(_ transitionContext: UIViewControllerContextTransitioning){
        if presented {
            let toView = transitionContext.view(forKey: UITransitionContextViewKey.to)
            toView!.layer.transform = CATransform3DMakeRotation(CGFloat(rotation.angle), rotation.x, rotation.y, rotation.z)
            UIView.animate(withDuration: self.duration, animations: {
            ...
            let fromView = transitionContext.view(forKey: UITransitionContextViewKey.from)
            UIView.animate(withDuration: self.duration, animations: {
                fromView!.layer.transform = CATransform3DMakeRotation(CGFloat(self.rotation.angle), self.rotation.x, self.rotation.y, self.rotation.z)
            }, completion: { (finished) in
                transitionContext.completeTransition(true)
            });
        }
